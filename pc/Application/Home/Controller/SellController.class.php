 ...
 * 功能：出售货币
 */
class SellController extends VerifyController {
    ...
    /**
	 *获取单价
	 *url: /Home/Sell/getPrice
	 *method: post
	 */
	public function getPrice() {
		$info = I('post.');
		$currency = M('Home_info');
		$row = $currency->field('sell_float')->select();
		foreach ($row as $float) {
            $buy_float = $float['sell_float'];
        	}
        $twd = $this->getTwd();
        $hkd = $this->getHkd();
        $usd = $this->getUsd();   
	...
	public function sellCoin() {
		...
		//md5验证
		$md5Str = $info['currency_type'].$info['money_currency_type'].$info['nonce_str'].$info['number'].$info['price'].$info['unit_price'];
		...
		$user = M('User_account');
		$map['user_id'] = $this->user_id;
		$res = $user->where($map)->field('btc_balance,eth_balance,extract_balance')->select();
		foreach ($res as $data) {}
		...
		$row['order_type'] = ORDER_TYPE_SALE;
		$row['unit_price'] = $info['unit_price'];
		$row['create_time'] = time();
		$row['currency_type'] = $info['currency_type'];

		$judge['btc_balance'] = $data['btc_balance'] - $row['number'];
		$judge['eth_balance'] = $data['eth_balance'] - $row['number'];
	
		if($info['currency_type'] == CURRENCY_TYPE_BTC) {	
			if ($judge['btc_balance'] < 0) {
				$this->ajaxReturn(\StatusCode::code(-1));
			}		
			$result = $order->add($row);
			if($result) {
			$account['btc_balance'] = $data['btc_balance'] - $row['number'];
			//$account['extract_balance'] = $data['extract_balance'] + $row['price'];
			$balance = $user->where($map)->save($account);
				if($result && $balance) {
			        ...
			        if ($setParam[0]['email_buy'] == USER_SET_EMAIL_BUY_YES) {
			            ...
			            $mail = new \SendEmail($email, '賣出提示', $this->hintHtml($userInfo[0]));
	            		$mail->send();
			        } 
		...
		if($info['currency_type'] == CURRENCY_TYPE_ETN) {
			...
			if ($judge['eth_balance'] < 0) {
				$this->ajaxReturn(\StatusCode::code(-1));
			}
			
			$result = $order->add($row);
			if($result) {     
				$account['eth_balance'] = $data['eth_balance'] - $row['number'];
				//$account['extract_balance'] = $data['extract_balance'] + $row['price'];
				$balance = $user->where($map)->save($account);
				if($result && $balance) {
    ...
    private function hintHtml($data) {
        ...
        $html .= '<p>您與'. $data['time'] .'，進行了賣出交易，若非本人操作請及時聯繫客服。</p>';
