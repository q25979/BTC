...
<script type="text/javascript">
    ...
    <div class="bankCard col-lg-8 col-md-8 col-sm-8 col-xs-12">
        <div style="margin-top: 20px;"><a href="http://localhost:8081/Home/Setting/index"> < <?php echo (L("_ACCOUNT_SETTINGS_BACK_")); ?></a></div>
        <br>
    ...
    <script type="text/javascript" src="http://localhost:8081/Public/plug-in/layui/layui.js"></script>
    <script type="text/javascript" src="http://localhost:8081/Public/plug-in/layer_mobile/layer.js"></script>
    ...
    // 判断是否开启验证码
    $.get('http://localhost:8081/Home/Setting/msgStatus', function(res) {
	...
	$("#SMS-verification-btn").click(function(){
		...
		if($("#bank_select").val() != "" && $("#branch").val() != "" && $("#bank-account-number").val() != "" && $("#bank-account-name").val() != ""){
			...
			var u = 'http://localhost:8081/Home/Setting/sendCode';
    ...
    function getdata(type,id){
		var vm = this;
		var url = "http://localhost:8081/Home/Setting/getBankInfo";
		var data = {
				type : type,
				bank_parent : id
		}
...
//				var userbankReg = /^\d{10,18}$/;
				...
				var u = 'http://localhost:8081/Home/Setting/Bankcard';
				var	d = {
					bank_name : bankName,
					bank_branch : bankSon,
				...
				$.ajax({
					...
					success: function (res) {
						...
						if(parseInt(res.code) == 0) {
		                    ...
		                    // 刷新
		                     window.location.replace("http://localhost:8081/Home/Setting/index?code=" + res.code);
