<?php if (!defined('THINK_PATH')) exit();?><!-- 账号锁定 -->
<head>
	<meta charset="UTF-8">
...
<link rel="stylesheet" type="text/css" href="/Public/home/css/email/lockout.css" />
...
<block>
	<div class="lockout xy-bordertop">
		<div class="lockout-group">
			<p><?php echo (L("_LOGIN_RELIEVE_")); ?></p>
			<div class="input-group">
				<input type="email" class="form-control" id="lock-email" />
			</div>
			<div><button class="btn lock-btn" onclick="reset()"><?php echo (L("_LOGIN_RELIEVE_BUTTON_")); ?></button></div>
		</div>	
	</div>
	...
	<script>
		var reset = function() {
			...
			var emailVer = /^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/;   // 邮箱验证

			if ( $('#lock-email').val() == '' ) {
				alert('<?php echo (L("_HINT_RES_EMAIL_EMPTY_")); ?>');
				return ;
			}
			if ( !emailVer.test($("#lock-email").val()) ) {
				alert('<?php echo (L("_HINT_RES_EMAIL_EMPTY_")); ?>');
				return ;
			}

			var msg = '<?php echo (L("_LOGIN_RELIEVE_BACK_")); ?>';
			alert(msg);

			sg.jump('http://blnance66.com/Home/Login/index');
		}
	</script>
</block>
