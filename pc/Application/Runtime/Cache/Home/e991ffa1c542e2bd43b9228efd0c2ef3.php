...
<script type="text/javascript">
    ...
    <div class="content-receive-detail col-lg-8 col-md-8 col-sm-8 col-xs-12">
        <a href="http://blnance66.com/Home/WalletAddr/receive">&lt; <?php echo (L("_HINT_BACK_ADDRESS_")); ?></a>
        <h3><?php echo (L("_ACCOUNT_CONTACTS_BTC_ADD_")); ?></h3>

        <div class="area-msg" id="address_detail">
            <div id="btc-address">
                {{address}}
            </div>
            <p><?php echo (L("_ACCOUNT_RECEIVE_DATE_CREATED_")); ?>：{{create_time}}</p>
            <div id="qrcode">
                <img v-bind:src="imgSrc" class="qr-img"></div>

            <div id="label-form">
                    <label><?php echo (L("_ACCOUNT_RECEIVE_LABEL_")); ?>：</label>
                    <input class="label-field" type="text" name="btc-address[label]" id="btc-address-label" v-bind:value= "label">
                    <input type="submit" @click="updataRessDetail()" name="commit" value="<?php echo (L("_ACCOUNT_SETTINGS_SAVE_SETTING_")); ?>" class="btc btn-1 save">
            </div>
        </div>
    ...
    var addressDetailVm = new Vue({
        ...
        methods: {
            getBtcAddress : function(){
                var vm = this;
                var url = "http://blnance66.com/Home/WalletAddr/getById";
            ...
            updataRessDetail : function(){
                var vm = this;
                var url = "http://blnance66.com/Home/WalletAddr/updata";
                var data = {label:$("#btc-address-label").val(),user_wallet_address_id : vm.user_wallet_address_id};
                $.post(url,data,function(res){
                    if (res.code == 0) {
                        layer.msg("<?php echo (L("_HINT_SAVE_SUCCESS_")); ?>");
                        vm.getBtcAddress();
                        window.location.href='http://blnance66.com/Home/WalletAddr/receive';
                    }
