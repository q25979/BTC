...
<script type="text/javascript">
        ...
        <form action="">
            <div class="sendstate-1">
                <div class="send-dep">
                    ...
                    <span class="buy-more"><a href="http://192.168.0.128:8081/Home/Buy/index">(<?php echo (L("_ACCOUNT_SEND_MORE_")); ?>)</a></span>
                </div>
    ...
    <script>
		...
		function getSendBalance(id){
			var SendBalance_url = "http://192.168.0.128:8081/Home/Send/getBalance";
			...
			if(parseFloat(BTC_unit_balance) >= parseFloat(number) && number != 0 && price != 0  && number > 0 && price > 0){
				...
				getSend(1,send_address,maincoin_id,number,leave_words,nonce_str, function(res) {
                    ...
                    if (res.code == 0) {
                        ...
                        window.location.href = "http://192.168.0.128:8081/Home/Index/index";
			...
			if(parseFloat(ETH_unit_balance) >= parseFloat(number) && number != 0 && price != 0  && number > 0 && price > 0){
				...
				getSend(2,send_address,maincoin_id,number,leave_words,nonce_str,function(res){
					...
					if(res.code == 0){
						...
						window.location.href = "http://192.168.0.128:8081/Home/Index/index?type=2";
					}else{
						// 会失败
...
//						,bindAction: '#submit-send-form' //指向一个按钮触发上传
					,url: 'http://192.168.0.128:8081/home/Send/getUpload' //上传接口
					,done: function(res){
						 layer.closeAll();
						//如果上传失败
						if(res.code > 0) {
		...
		function getSend(id,send_address,maincoin_id,number,leave_words,nonce_str, callback){
			var Send_url = "http://192.168.0.128:8081/Home/Send/getSendInfo";
			var Send_data = {
				currency_type: id,
