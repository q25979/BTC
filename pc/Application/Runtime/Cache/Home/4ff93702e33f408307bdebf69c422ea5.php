...
<script type="text/javascript">
	...
	<div class="content-security col-lg-8 col-md-8 col-sm-8 col-xs-12">
		<div class="security">
			...
			<div class="section password-section">
				<h3><?php echo (L("_ACCOUNT_SECURITY_ACCOUNT_PWS_")); ?></h3>
				<div class="fr"><i class="icon icon-success" style="background: url(/Public/images/safe-ioc.png) no-repeat scroll 0px 9px transparent;"></i></div>
				<div class="password-content">
					<p><?php echo (L("_ACCOUNT_SECURITY_PWS_")); ?> <b>●●●●●●●●●●●●●●●●</b></p>
					<p class="password-strength" data-score="0">
						<?php echo (L("_ACCOUNT_SECURITY_PWS_STRENGTH_")); ?>
						<!--安全强度-->	
						<span></span>
						<span></span>
						<span></span>
					...
					<p>
						...
						<a href="http://blnance66.com/Home/Security/updatePassword"><?php echo (L("_ACCOUNT_SECURITY_CHANGE_PWD_")); ?></a>
					</p>
				</div>
			</div>
			<div class="dash"></div>
			...
			<div class="section security-qa-section">
				<h3><?php echo (L("_ACCOUNT_SECURITY_QUESTIONS_")); ?></h3>
				<div class="fr"><i class="icon icon-success" id="security-icon"></i></div>
				<div class="content">
					<!--<div class="fr"><i class="icon icon-alert"></i></div>-->
					<p><?php echo (L("_ACCOUNT_SECURITY_QUESTIONS_TIPS_")); ?></p>
					<p>
						<span class="link" style="display:none;">
							<a id="security-a" href="http://blnance66.com/Home/Security/enableSecurity"><?php echo (L("_ACCOUNT_SECURITY_OPEN_QUESTIONS_")); ?></a>
						</span>
					</p>
				</div>
			</div>
	...
	<script>
		...
		//显示数据
		$.ajax({
		url: 'http://blnance66.com/Home/Security/Passwordstrength',
		success: function (res) {
			$('.link').css({ 'display': 'block' });
            //密保问题设定后 链接隐藏，并且换图标
			if(parseInt(res.data.num) == 0){
                $('#security-a').hide();
				$('#security-icon').css({ "background": "url(/Public/images/safe-ioc.png) no-repeat 0px 9px scroll transparent"})
			}else{
				$('#security-a').show();
			};
			//安全强度
...
//				$('#strength-1').addClass("strength0");
				...
				$('.password-score').html('<?php echo (L("_HINT_PWD_STRENGTH_WEAK_")); ?>');
			}
			if(res.data.strength==2){
//				$('#strength-1').addClass("strength0");
//				$('#strength-2').addClass("strength0");
                $('.password-strength span:lt(2)').addClass("strength0");
				$('.password-score').html('<?php echo (L("_HINT_PWD_STRENGTH_WEAKER_")); ?>');
			}
			if(res.data.strength==3){

                $('.password-strength span:lt(3)').addClass("strength0");
				$('.password-score').html('<?php echo (L("_HINT_PWD_STRENGTH_MIDDLE_")); ?>');
			}
			if(res.data.strength==4){

                 $('.password-strength span:lt(4)').addClass("strength0");
				$('.password-score').html('<?php echo (L("_ACCOUNT_SECURITY_STRENGTH_TIPS_")); ?>');
			}
			if(res.data.strength==5){

				$('.password-strength span:lt(5)').addClass("strength0");
				$('.password-score').html('<?php echo (L("_HINT_PWD_STRENGTH_GOODER_")); ?>');
			}
		    //安全指数百分比			
			var percentage = res.data.percentage;
			var verbal ='<?php echo (L("_ACCOUNT_SECURITY_RANK_")); ?>'+percentage+'%';
...
//	});
	...
	<script type="text/javascript">
        ...
        $(".sidebar-content-a img").eq(2).attr("src","/Public/images/safety-click.png");
	</script>
