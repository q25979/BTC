...
$sql="select * from `wp_balance` where balance_sn=?";
$rs = $m->getOne( $sql, array('s', $order_id), false); //$rs返回查询到的结果 没有结果则返回false
if(!$rs)exit('not order');
$price=$rs["bpprice"];
$user=$rs["uid"];
...
$param = $rs["balance_sn"]; //自定义参数  可以留空 传递什么返回什么 用于区分游戏分区或用户身份
 ...
 * 查询1条信息演示代码
 * $order_id=$_GET['order_id'];
 * $sql="select * from `codepay_order` pay_no=?";
 * $rs = $m->getOne( $sql, array('s', $order_id), false); //$rs返回查询到的结果 没有结果则返回false
 *
 *
 */
